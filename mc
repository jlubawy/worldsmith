#!/usr/bin/env bash
# Run the Bedrock server at home or in the cloud, and move worlds between them.
# `./mc help` for usage.
set -euo pipefail
...
cmd_setup() {
  ...
  load_config
  local server_name allow ops tz lan dns_name dns_zone="" dns_project="" region

  echo "Saved to $CONFIG (not committed). Enter keeps the [value]; - clears it."
  echo
  prompt server_name "Server name in the server list" "${SERVER_NAME:-My Minecraft Server}"
