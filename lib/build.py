...
from bedrock import FACINGS, ask, players, send, where
...
# -- keeping the area loaded ------------------------------------------------------

class _Loaded:
    """Ticking area over a region, so it's loaded even with no player nearby."""

    def __init__(self, lo: Point, hi: Point):
        self.lo, self.hi = lo, hi

    def __enter__(self):
        ask("tickingarea remove mcbuild")  # leftover from an interrupted run
        reply = ask(f"tickingarea add {self.lo[0]} {self.lo[1]} {self.lo[2]} "
                    f"{self.hi[0]} {self.hi[1]} {self.hi[2]} mcbuild")
        if "ticking area" not in reply.lower() or ERROR_WORDS.search(reply.splitlines()[0]):
            raise SystemExit(f"couldn't load the area: {reply.splitlines()[0]}")
        time.sleep(2)  # give the chunks a moment to load
        return self

    def __exit__(self, *exc):
        ask("tickingarea remove mcbuild")
...
def run(design_name: str, player: str | None, distance: int | None, dry_run: bool, up: int = 0) -> int:
    ...
    with _Loaded(lo, hi):
        ...
        # Send everything, then read the replies back from the log in one go.
        window_start = time.time() - 5
        from bedrock import _logs_since  # noqa: PLC0415 (only needed here)
        already = len(_logs_since(window_start))
        for command in site.commands:
            send(command)
        time.sleep(1.5)
        replies = _logs_since(window_start)[already:]
...
def main(argv: list[str]) -> int:
    ...
    return run(args[0], args[1] if len(args) > 1 else None, distance, dry_run, opts["--up"])
