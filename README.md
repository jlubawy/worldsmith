...
Run a server for Minecraft: Bedrock Edition either at home (Docker on your
computer) or in the cloud (an AWS Lightsail server made with Pulumi). You can
move worlds back and forth between them, and build things in them from
scripts. Everything goes through `./mc`. Run `./mc help` for the full list of
commands.

> **Not an official Minecraft product. Not approved by or associated with
> Mojang or Microsoft.** See [Legal](#legal).
...
`setup` first asks you to accept Mojang's
[End User License Agreement](https://www.minecraft.net/eula). The server
software is theirs, and it won't start until you have. Then it asks for a
server name, the gamertags allowed on the cloud server, a time zone, your LAN
IP, and optionally a DNS name and an AWS region. It
...
| --- | --- | --- |
| `config.env` | EULA acceptance, server name, allowlist, operators, time zone | yes |
...
LICENSE               MIT, for this project's own files
...
Minecraft is a trademark of Mojang Synergies AB. This project isn't affiliated
with, endorsed by or sponsored by Mojang or Microsoft, and it follows the
[Minecraft Usage Guidelines](https://www.minecraft.net/usage-guidelines).

This repository doesn't contain or redistribute any Minecraft software. The
[itzg/minecraft-bedrock-server](https://github.com/itzg/docker-minecraft-bedrock-server)
Docker image downloads Mojang's Bedrock Dedicated Server when the container
first starts. Running it is subject to Mojang's
[End User License Agreement](https://www.minecraft.net/eula), which each user
accepts for themselves in `./mc setup`. Nothing here accepts it for you.

The code in this repository is released under the [MIT License](LICENSE). That
covers only this project's own files, not Minecraft or the Docker images it
uses.
